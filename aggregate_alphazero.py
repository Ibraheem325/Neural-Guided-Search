...
def aggregate(output_dir, csv_path):
    ...
    for fname in sorted(os.listdir(output_dir)):
        ...
        sol_len, sims, generated = None, None, None
        ...
        results.append([problem, solved, sol_len, sims, generated])
    ...
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['Problem', 'Solved', 'Solution Length', 'Simulations', 'Generated'])
        for r in results:
            w.writerow(r)
        w.writerow([])
        w.writerow(['SUMMARY', f'{len(solved)}/{total}', avg_len, avg_sims, ''])
        w.writerow(['Coverage', f'{coverage}%', '', '', ''])
    
    print(f'{csv_path}: {coverage}% coverage ({len(solved)}/{total}), avg_len={avg_len}, avg_sims={avg_sims}')
