import os, re
configs = {
    'baseline'  : 'results/az_logi_w1tk_baseline',
    'lam10_all' : 'results/az_logi_w1tk_lam10_all',
    'lam10_top1': 'results/az_logi_w1tk_lam10_top1',
}
def parse(d):
    o={}
...
for k in configs:
    ...
    print(f"  {k:12s}: {s}/{n} ({100*s/max(1,n):.1f}%)")
common=None
for k in configs:
...
print(f"{'config':12s} {'avg_exp':>10} {'avg_len':>8}")
for k in configs:
    d=data[k]; e=[d[n][1] for n in common if d[n][1] is not None]; l=[d[n][2] for n in common if d[n][2] is not None]
    print(f"{k:12s} {sum(e)/len(e) if e else 0:10.1f} {sum(l)/len(l) if l else 0:8.1f}")
base=data['baseline']
print("\nvs baseline:")
for k in configs:
