...
def _plan(
    ...
    with torch.no_grad():
        ...
        while not goal.holds(current_state) and (len(solution) < 1_000):
            ...
            if use_closed_set:
                action_scores = mask_visited_scores(successor_states, action_scores, visited_states, float('-inf'))
...
def _main(args: argparse.Namespace) -> None:
    ...
    solution = _plan(problem, model, policy_model, use_closed_set=not args.disable_closed_set)
    if solution is None:
