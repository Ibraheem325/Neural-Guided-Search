...
def analyze(problem_path, domain, policy_model, iqn_model, taus):
    ...
    pcurve, pmean = curve_and_mean(iqn_model, state, goal, taus)

    rows = []
    for action, p in zip(actions, probs):
        succ = action.apply(state)
        ccurve, cmean = curve_and_mean(iqn_model, succ, goal, taus)
    ...
    return dict(
        n=len(rows), pmean=pmean,
        dom_prior=dom["prior"], dom_mag=dom["mag"], dom_signed=dom["signed"],
        dom_cmean=dom["cmean"], dom_action=dom["action"],
...
def main():
    ...
    with torch.no_grad():
        for prob_path in args.problems:
            ...
            verdict = ("ALT > DOM  <- signal could redirect" if r["max_alt_mag"] > r["dom_mag"]
                       else "DOM highest  <- signal cannot redirect")
            print(f"  {r['n']} successors | parent IQN mean: {r['pmean']:.3f}", flush=True)
            print(f"  Dominant: prior={r['dom_prior']:.4f}  W1={r['dom_mag']:.4f}"
                  f"  child_mean={r['dom_cmean']:.3f}", flush=True)
            print(f"  Max-alt W1 (unsigned): {r['max_alt_mag']:.4f}"
                  f"  (signed): {r['max_alt_signed']:.4f}  CV={r['cv']:.3f}", flush=True)
            ...
            for row in r["rows"][:5]:
                ...
                print(f"    p={row['prior']:.4f}  W1={row['mag']:.4f}"
                      f"  signed={row['signed']:+.4f}  child_mean={cm}"
                      f"  {row['action']}", flush=True)

            if r["max_alt_mag"] > r["dom_mag"]:
    ...
    print(f"  Alt W1 > Dom W1 (signal could redirect): {can_help or 'none'}", flush=True)
