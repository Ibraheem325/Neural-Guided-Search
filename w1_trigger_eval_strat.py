...
def _count_objects_typed(problem_path):
...
def collect(domain, model, instances, taus, seed, cap, maxi, sper):
    ...
    with torch.no_grad():
        for f in files:
            ...
            nobj_f = _count_objects_typed(str(f))
            ...
            for s in states:
                ...
                DEP.append(float(ls.steps_to_goal)); NOBJ.append(float(nobj_f)); taken += 1
            if used >= maxi: break
    return map(np.array, (W, ERR, DEP, NOBJ))
...
def _lift_in(mask, W, ERR, he, base, k=0.10):
    if mask.sum() == 0: return float("nan"), 0
    Wm, Em = W[mask], ERR[mask]
    thr = np.quantile(Wm, 1 - k); sel = Wm >= thr
    if sel.sum() == 0: return float("nan"), 0
    return ((Em[sel] >= he).mean() / base if base > 0 else float("nan")), int(sel.sum())

def report_stratified(tag, W, ERR, DEP, NOBJ, hq):
    he = np.quantile(ERR, hq); base = (ERR >= he).mean()
    print(f"\n========  {tag}: STRATIFIED top-10% lift  ========")
    print(f"states={len(W)}  base high-err rate={100*base:.1f}%")
    print(f"object count: min={NOBJ.min():.0f} max={NOBJ.max():.0f} mean={NOBJ.mean():.1f}")
    print(f"goal distance: min={DEP.min():.0f} max={DEP.max():.0f} mean={DEP.mean():.1f}")
    ov, n = _lift_in(np.ones(len(W), bool), W, ERR, he, base); print(f"OVERALL lift={ov:.2f} (n_hi={n})")
    o1, o2 = np.quantile(NOBJ, [1/3, 2/3])
    print("by OBJECT COUNT:")
    for nm, m in [('small', NOBJ<=o1), ('med', (NOBJ>o1)&(NOBJ<=o2)), ('large', NOBJ>o2)]:
        l, n = _lift_in(m, W, ERR, he, base)
        rng = f"{NOBJ[m].min():.0f}-{NOBJ[m].max():.0f}" if m.sum() else "-"
        print(f"  {nm:6s} (obj {rng}): lift={l:.2f} (n={int(m.sum())}, n_hi={n})")
    g1, g2 = np.quantile(DEP, [1/3, 2/3])
    print("by GOAL DISTANCE:")
    for nm, m in [('shallow', DEP<=g1), ('mid', (DEP>g1)&(DEP<=g2)), ('deep', DEP>g2)]:
        l, n = _lift_in(m, W, ERR, he, base)
        rng = f"{DEP[m].min():.0f}-{DEP[m].max():.0f}" if m.sum() else "-"
        print(f"  {nm:7s} (gd {rng}): lift={l:.2f} (n={int(m.sum())}, n_hi={n})")
...
def main():
    ...
    for sd in args.seeds:
        W, E, D, _NOBJ = collect(domain, model, args.instances, taus, sd,
                          args.state_cap, args.max_instances, args.states_per_instance)
        report(f"SEED {sd}", W, E, D, args.high_quantile); AW.append(W); AE.append(E); AD.append(D); ANOBJ.append(_NOBJ)
    PW, PE, PD, PN = np.concatenate(AW), np.concatenate(AE), np.concatenate(AD), np.concatenate(ANOBJ)
    if len(args.seeds) > 1:
        report("POOLED", PW, PE, PD, args.high_quantile)
    report_stratified("POOLED" if len(args.seeds) > 1 else f"SEED {args.seeds[0]}", PW, PE, PD, PN, args.high_quantile)
