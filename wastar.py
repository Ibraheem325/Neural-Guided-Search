...
class WeightedNeuralHeuristic(mm.Heuristic):
    def __init__(self, model: rgnn.RelationalGraphNeuralNetwork, weight: float):
        super().__init__()
        self._model = model
        self._weight = weight
    ...
    def compute_value(self, state: mm.State, goal: Union[mm.GroundConjunctiveCondition, None] = None) -> float:
        ...
        with torch.no_grad():
            ...
            return float(value.item())*self._weight
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--weight', default=1.0, type=float, help='Weights set to the heuristic')
...
def _main(args: argparse.Namespace) -> None:
    ...
    weighted_neural_heuristic = WeightedNeuralHeuristic(model, args.weight)
